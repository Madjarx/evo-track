...
1. Initialize passwords for built-in users with one of the 2 following methods:

    ```console
    $ docker-compose exec -T elasticsearch bin/elasticsearch-setup-passwords auto --batch 
    ```
    auto - creates randomly generated passwords
    ...
    Alternatively, do a [post request](https://www.elastic.co/guide/en/elasticsearch/reference/current/security-api-change-password.html) to change the password:
    ...
    ```Unexpected response code [429] from calling PUT http://172.25.0.2:9200/_security/user/apm_system/_password?pretty
      Cause: index [.security-7] blocked by: [TOO_MANY_REQUESTS/12/disk usage exceeded flood-stage watermark, index has read-only-allow-delete block];
    ```

    Elasticsearch was installed a read-only mode likely because there is less that 5% of free disk space. To return to the read-write mode, run the following:

    ```
    $ curl -XPUT -u "<username>:<password>" -H "Content-Type: application/json" http://localhost:9200/_cluster/settings -d '{ "transient": { "cluster.routing.allocation.disk.threshold_enabled": false } }'
    $ curl -XPUT -u "<username>:<password>" -H "Content-Type: application/json" http://localhost:9200/_all/_settings -d '{"index.blocks.read_only_allow_delete": null}'
    ```

    (username is likely `elastic` and password is likely `changeme`)
